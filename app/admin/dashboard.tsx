...
import { createStudentAction, updateStudentAction, recordAttendanceAction, addAchievementAction, deleteAchievementAction, logoutAction } from './actions';
...
export default function AdminDashboard({ students, attendance, achievements }: Props) {
    ...
    <main style={{ minHeight: '100vh', background: '#fafaf8' }}>
      <header style={{ background: '#1a472a', color: 'white', padding: '16px 0' }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <form action={logoutAction}>
            <button type="submit" style={{ fontSize: '14px', color: 'rgba(255,255,255,0.7)', background: 'none', border: 'none', cursor: 'pointer' }}>Logout</button>
          </form>
        </div>
      </header>
      ...
      <div className="container" style={{ padding: '24px 16px' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <button onClick={() => setTab('students')} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '14px', fontWeight: 500, background: tab === 'students' ? '#1a472a' : 'white', color: tab === 'students' ? 'white' : '#475569', border: '1px solid #e2e8f0', cursor: 'pointer' }}>Students</button>
          <button onClick={() => setTab('attendance')} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '14px', fontWeight: 500, background: tab === 'attendance' ? '#1a472a' : 'white', color: tab === 'attendance' ? 'white' : '#475569', border: '1px solid #e2e8f0', cursor: 'pointer' }}>Attendance</button>
          <button onClick={() => setTab('achievements')} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '14px', fontWeight: 500, background: tab === 'achievements' ? '#1a472a' : 'white', color: tab === 'achievements' ? 'white' : '#475569', border: '1px solid #e2e8f0', cursor: 'pointer' }}>Achievements</button>
        </div>
        ...
        {tab === 'students' && (
          <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '20px' }}>
            ...
            <form action={async (formData) => { const r = await createStudentAction(formData); if (r.success) setMessage('Student added'); }} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button type="submit" style={{ padding: '12px', borderRadius: '8px', background: '#1a472a', color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer' }}>Add Student</button>
            </form>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {students.map((student) => (
                <div key={student.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9', flexWrap: 'wrap', gap: '8px' }}>
                  ...
                  <form action={async (formData) => { const r = await updateStudentAction(formData); if (r.success) setMessage('Student updated'); }} style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button type="submit" style={{ padding: '6px 12px', borderRadius: '6px', background: '#1a472a', color: 'white', fontSize: '12px', border: 'none', cursor: 'pointer' }}>Save</button>
                  </form>
                </div>
              ))}
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {students.filter((s) => s.is_active && s.joining_date !== null).map((student) => {
                ...
                return (
                  <div key={student.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                    <span style={{ fontSize: '14px', fontWeight: 500 }}>{student.full_name}</span>
                    <button onClick={() => toggleStatus(student.id)} style={{ padding: '6px 16px', borderRadius: '999px', border: 'none', fontWeight: 600, fontSize: '14px', cursor: 'pointer', background: statusColor(currentStatus), color: 'white' }}>
            ...
            <button onClick={saveAttendance} style={{ marginTop: '16px', width: '100%', padding: '12px', borderRadius: '8px', background: '#1a472a', color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer' }}>Save Attendance</button>
        ...
        {tab === 'achievements' && (
          <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '20px' }}>
            ...
            <form action={async (formData) => { const r = await addAchievementAction(formData); if (r.success) setMessage('Achievement added'); }} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button type="submit" style={{ padding: '12px', borderRadius: '8px', background: '#1a472a', color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer' }}>Add Achievement</button>
            </form>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {achievements.map((a) => (
                <div key={a.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <form action={async (formData) => { const r = await deleteAchievementAction(formData); if (r.success) setMessage('Achievement deleted'); }}>
                    <input type="hidden" name="id" value={a.id} />
                    <button type="submit" style={{ fontSize: '12px', color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer' }}>Delete</button>
                  </form>
                </div>
              ))}
            </div>
