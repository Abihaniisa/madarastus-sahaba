...
export async function addStudent(formData: FormData) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const { error } = await supabaseAdmin.from('students').insert({ id: newId, full_name, joining_date: joining_date || null, joining_week });
...
export async function updateStudent(formData: FormData) {
  ...
  if (!id || !full_name) redirect('/admin');
  await supabaseAdmin.from('students').update({ full_name, joining_week, is_active }).eq('id', id);
  redirect('/admin');
}

export async function saveAttendanceBatch(records: Array<{ student_id: string; date: string; status: string }>) {
  await requireAdmin();
  for (const record of records) {
    if (!['R', 'M', 'X'].includes(record.status)) return { error: 'Invalid status' };
  }
  const { error } = await supabaseAdmin.from('attendance_records').upsert(records, { onConflict: 'student_id,date' });
...
export async function addAchievement(formData: FormData) {
  ...
  if (!student_id || !title) redirect('/admin');
...
export async function saveAnnouncement(formData: FormData) {
  ...
  if (!arabic_text && !english_text && !schedule) redirect('/admin');
  await supabaseAdmin.from('announcements').insert({ arabic_text, english_text, schedule });
  redirect('/admin');
}
...
export async function uploadStudentPhoto(formData: FormData) {
  ...
  if (!student_id || !file || file.size === 0) return { error: 'Missing student or file.' };
  if (!file.type.startsWith('image/')) return { error: 'Only image files are allowed.' };
  if (file.size > 5 * 1024 * 1024) return { error: 'Image size must be under 5MB.' };
  ...
  const { error: updateError } = await supabaseAdmin.from('students').update({ photo_url: urlData.publicUrl }).eq('id', student_id);
...
export async function removeStudentPhoto(formData: FormData) {
  ...
  const { data: student } = await supabaseAdmin.from('students').select('photo_url').eq('id', student_id).single();
  if (student?.photo_url) {
    const url = student.photo_url;
    const path = url.split('/').pop();
    if (path) {
      await supabaseAdmin.storage.from('student-photos').remove([`students/${path}`]);
    }
  }
  const { error } = await supabaseAdmin.from('students').update({ photo_url: null }).eq('id', student_id);
  if (error) return { error: error.message };
  return { success: true };
}
